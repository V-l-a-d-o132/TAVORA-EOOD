...
export default function LessonEngineV2({
  ...
  return <div className="mx-auto flex min-h-full w-full max-w-5xl flex-col pb-24 text-white" data-testid="lesson-engine-v2">
    <header className="mb-5 rounded-3xl border border-white/10 bg-gradient-to-br from-[#181818] to-[#0d0d0d] p-5 sm:p-7">
      <div className="flex flex-wrap items-center justify-between gap-3"><span className="text-xs font-bold uppercase tracking-[.22em] text-red-400">Lesson Engine V2 · {lesson.duration}</span><span aria-live="polite" className="text-xs text-zinc-500">{saveStatus === 'saving' ? 'Запазване…' : saveStatus === 'saved' ? 'Запазено' : saveStatus === 'error' ? 'Проблем при autosave' : userId ? `Autosave · ${xp} XP · ${masteryStatus}` : 'Preview без запис'}</span></div>
      <h1 className="mt-3 text-2xl font-semibold leading-tight sm:text-3xl">{lesson.title}</h1>
      {lesson.subtitle && <p className="mt-2 text-sm leading-6 text-zinc-400">{lesson.subtitle}</p>}
      <div className="mt-6"><div className="mb-2 flex items-center justify-between text-xs text-zinc-400"><span>Умения и задачи</span><span>{progressPercent}% · {completed.size}/{lesson.blocks.length}</span></div><div className="h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-gradient-to-r from-red-600 to-red-400 transition-[width] motion-reduce:transition-none" style={{ width: `${progressPercent}%` }} /></div></div>
    </header>

    <section aria-labelledby="skills-heading" className="mb-5 rounded-2xl border border-emerald-500/15 bg-emerald-500/[0.045] p-4 sm:p-5">
      <h2 id="skills-heading" className="text-sm font-semibold text-emerald-200">Какво вече умея</h2>
      {completed.size === 0 ? <p className="mt-2 text-sm text-zinc-500">Завърши практическа стъпка, за да видиш доказаните умения тук.</p> : <ul className="mt-3 flex flex-wrap gap-2">{lesson.blocks.filter((block) => completed.has(block.key)).map((block) => <li key={block.key} className="rounded-full border border-emerald-500/20 bg-emerald-500/10 px-3 py-1.5 text-xs text-emerald-100"><i className="ri-check-line mr-1" />{block.title}</li>)}</ul>}
      {masteryStatus === 'mastered' && <p className="mt-3 text-sm font-medium text-white">Овладяно: {lesson.objective}</p>}
    </section>

    <nav aria-label="Стъпки на урока" className="mb-5 flex gap-2 overflow-x-auto pb-2">{lesson.blocks.map((block, index) => <button key={block.key} type="button" aria-label={`Стъпка ${index + 1}: ${block.title}`} aria-current={index === currentIndex ? 'step' : undefined} onClick={() => goTo(index)} className={`h-2 min-w-8 flex-1 rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-red-400 ${index === currentIndex ? 'bg-red-500' : completed.has(block.key) ? 'bg-emerald-500' : 'bg-white/15'}`} />)}</nav>
