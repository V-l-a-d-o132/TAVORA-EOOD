...
export default function LessonBlockRenderer({ block, lessonId, initialState, completed, onStateChange, onSubmit }: Props) {
  ...
  return <section aria-labelledby={`block-${block.key}`} className="rounded-3xl border border-white/10 bg-[#111]/95 p-5 shadow-2xl shadow-black/20 sm:p-7 md:p-9">
    <header className="mb-6 flex items-start justify-between gap-4"><div><span className="text-[11px] font-bold uppercase tracking-[.2em] text-red-400">{block.type.replaceAll('_', ' ')}</span><h2 id={`block-${block.key}`} className="mt-2 text-2xl font-semibold leading-tight text-white">{block.title}</h2></div><span aria-label={`${block.points} XP`} className="shrink-0 rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs font-semibold text-zinc-400">+{block.points} XP</span></header>
    {renderBlock()}
